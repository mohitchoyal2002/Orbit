import { SITE_ORIGINS, surfaceForHostname } from "@/lib/site-domains";

export function GET(request: Request) {
  const origin = SITE_ORIGINS.marketing;
  const urls = surfaceForHostname(new URL(request.url).hostname) === "marketing" ? `<url><loc>${origin}/</loc></url><url><loc>${origin}/privacy</loc></url>` : "";
  return new Response(`<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urls}</urlset>`, { headers: { "Content-Type": "application/xml; charset=utf-8", "Cache-Control": "public, max-age=3600" } });
}
